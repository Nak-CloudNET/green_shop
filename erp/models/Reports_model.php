...
ORDER BY
    ...
    {
        $myQuery = "SELECT DATE_FORMAT( erp_sales.date,  '%e' ) AS date, SUM( COALESCE( erp_sales.product_tax, 0 ) ) AS tax1, SUM( COALESCE( erp_sales.order_tax, 0 ) ) AS tax2, SUM( COALESCE( erp_sales.total, 0 ) ) AS total, SUM( COALESCE( erp_sales.total_discount, 0 ) ) AS discount, SUM( COALESCE( erp_sales.shipping, 0 ) ) AS shipping,SUM(COALESCE(erp_return_sales.grand_total,0)) as t_return
            FROM " . $this->db->dbprefix('sales') . " LEFT JOIN erp_return_sales ON erp_return_sales.sale_id = erp_sales.id
...
*/
   ...
   public function getCustomersByID($id=null, $wh=null) {
	...
	public function getWarehouseFromPurchase($wid,$warehouse,$product,$category,$biller,$start,$end){

        $w = "";
        if ($warehouse) {
            $w .= " AND warehouse_id = '{$warehouse}'";
        } else {
            if($wid){
                $w .= " AND warehouse_id = '{$wid}'";
            }
        }

        if ($product) {
            $w .= " AND product_id = '{$product}'";
        }
        ...
        $q = $this->db->query("SELECT c.*,erp_categories.id as category_id,erp_categories.name as category_name,erp_warehouses.name as warehouse_name,erp_products.name as product_name FROM (
        SELECT
            warehouse_id,product_id,
            SUM( IFNULL( IF ( transaction_type = 'purchase', quantity_balance, 0 ), 0 ) ) AS t_in_purchase,
            SUM( IFNULL( IF ( quantity_balance > 0 and transaction_type = 'adjustment', quantity_balance, 0 ), 0 ) ) AS t_in_adjustment,
            SUM( IFNULL( IF ( quantity_balance > 0 and transaction_type = 'transfer', quantity_balance, 0 ), 0 ) ) AS t_in_transfer,
                
                
            - SUM( IFNULL( IF ( transaction_type = 'sale', quantity_balance, 0 ), 0 ) ) AS t_out_sale,
            - SUM( IFNULL( IF ( quantity_balance < 0 and transaction_type = 'adjustment', quantity_balance, 0 ), 0 ) ) AS t_out_adjustment,
            - SUM( IFNULL( IF ( quantity_balance < 0 and transaction_type = 'transfer', quantity_balance, 0 ), 0 ) ) AS t_out_transfer
        FROM
            `erp_purchase_items`
        WHERE
            `erp_purchase_items`.`status` != 'pending' 
            AND `erp_purchase_items`.`date` >= '{$start}'
            AND `erp_purchase_items`.`date` <= '{$end}' 
             
        GROUP BY
            warehouse_id,product_id
        ) as c
        
          INNER JOIN erp_products on c.product_id = erp_products.id
          INNER JOIN erp_categories on erp_products.category_id = erp_categories.id
          INNER JOIN erp_warehouses on c.warehouse_id = erp_warehouses.id
          where 1 = 1 {$w} ");

        return $q->result();

	}
	public function getBegin($wid,$warehouse,$product,$category,$biller,$from_date,$to_date){
        $w = "";
        if ($warehouse) {
            $w .= " AND warehouse_id = '{$warehouse}'";
        } else {
        ...
        $q = $this->db->query("SELECT
            warehouse_id,
            product_id,
            SUM( IFNULL( quantity_balance, 0 ) ) AS beggin
        FROM
            `erp_purchase_items` 
        WHERE
            `erp_purchase_items`.`status` != 'pending' 
            AND `erp_purchase_items`.`date` < '{$from_date}' 
            {$w}
        GROUP BY
            warehouse_id,
            product_id ");
    ...
    public function geInventory_data($wid,$warehouse,$product,$category,$start,$end){
        ...
        SELECT
            warehouse_id,product_id
        FROM
            `erp_inventory_valuation_details`
        WHERE
            `erp_inventory_valuation_details`.`date` >= '{$start}'
            AND `erp_inventory_valuation_details`.`date` <= '{$end}' 
	...
	public function getProPur($wid,$cid,$product2,$biller,$cat_data_product){
		...
		$this->db->select("products.id,products.code,products.name,units.name as name_unit,products.category_id,0 as warehouse_id")
		//->join("products","products.id=purchase_items.product_id","LEFT")
		...
		$this->db->where_in('products.id',$cat_data_product);
	...
	public function getProPur_old($wid,$cid,$product2,$biller){
		$this->db->select("product_id,products.code,products.name,units.name as name_unit,products.category_id,purchase_items.warehouse_id")
		->join("products","products.id=purchase_items.product_id","LEFT")
		->join("units","units.id=products.unit","LEFT")
		//->join("erp_purchases","erp_purchases.id=purchase_items.purchase_id","LEFT")
	...
	*/	
		...
		if($category2){
			$www .= " AND erp_products.category_id = '{$category2}' ";
		}
		if($product2){
			
			$www .= " AND erp_products.id = '{$product2}' ";
		}
		
		
		$q = $this->db->query("SELECT  `erp_categories`.`id`, `erp_categories`.`name`,c.product_id FROM (
								SELECT
								erp_products.id as product_id,
								`erp_products`.`category_id`
								FROM
									`erp_products`
									INNER JOIN `erp_purchase_items` ON `erp_purchase_items`.`product_id` = `erp_products`.`id` 
								WHERE
									`erp_purchase_items`.`status` != 'pending' 
									AND `erp_purchase_items`.`product_type` != 'service' 
									AND `erp_purchase_items`.`warehouse_id` = '{$wid}' {$www}
								GROUP BY
									erp_products.id,
									`erp_products`.`category_id`
									
									) as c 
								INNER JOIN erp_categories on c.category_id = `erp_categories`.`id` ");
		...
		if ($q->num_rows() > 0) {
            ...
            foreach (($q->result()) as $row) {
                $cat_data[$row->id] = $row;
                $cat_data_product[$row->id][$row->product_id] = $row->product_id;
            }
            return ['cat_data' => $cat_data,'cat_data_product'=>$cat_data_product];
        }
        return FALSE;
	}
	
	public function getProCat_old($wid,$category2,$product2,$biller){
		$this->db->select("erp_categories.id,erp_categories.name")
				 ->join("erp_categories","erp_categories.id=products.category_id","LEFT")
				 ->join("purchase_items","purchase_items.product_id=products.id","LEFT")
				 //->join("erp_purchases","erp_purchases.id=purchase_items.purchase_id","LEFT")
	...
	public function getTransuctionsPurIN_old($product2,$warehouse2,$start,$end,$biller){
		$this->db->select("transaction_type");
		$this->db->join("erp_purchases","erp_purchases.id=purchase_items.purchase_id","LEFT");
		$this->db->where("quantity_balance > ",0);
		if($product2){
	...
	public function getTransuctionsPurOUT_old($product2,$warehouse2,$start,$end,$biller){
		$this->db->select("transaction_type");
		$this->db->join("erp_purchases","erp_purchases.id=purchase_items.purchase_id","LEFT");
		$this->db->where("quantity_balance<",0);
		if($product2){
	...
	public function getBeginQtyINALL($id,$wid,$start,$end,$biller){
		...
		$this->db->select("SUM(COALESCE(quantity_balance,0)) as bqty,erp_purchase_items.product_id,erp_purchase_items.warehouse_id");
		$this->db->join("erp_purchases","erp_purchases.id=purchase_items.purchase_id","LEFT");
		$this->db->where("quantity_balance>",0);
		if($biller){
			$this->db->where("erp_purchases.biller_id",$biller);
		...
		//$this->db->where(array("purchase_items.product_id"=>$id,"purchase_items.warehouse_id"=>$wid));
		$this->db->where(array("purchase_items.warehouse_id"=>$wid));
		$this->db->group_by('erp_purchase_items.product_id,erp_purchase_items.warehouse_id');
		
		$q = $this->db->get("purchase_items");
		$data = [];
		if ($q->num_rows() > 0) {
            foreach($q->result() as $row){
				$data[$row->warehouse_id][$row->product_id] = $row->bqty;
			}
