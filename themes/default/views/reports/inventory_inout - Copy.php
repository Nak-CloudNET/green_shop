                        ...
                        <tbody>
							...
							?>		
								...
								<?php
									...
									if(isset($procat['cat_data'])){
									foreach($procat['cat_data'] as $rc){	
								...
								<?php
									...
									//====================================
									//====================================
									$cat_data_product = isset($procat['cat_data_product'][$rc->id])?$procat['cat_data_product'][$rc->id]:[];
									//====================================
									//====================================
									
									
									
									$propur = $this->reports_model->getProPur($rw->id,$rc->id,$product2,$biller2,$cat_data_product);
									
									$beginINqty = $this->reports_model->getBeginQtyINALL(0,$rw->id,$from_date2,$to_date2,$biller2);
									
									$beginOUTqty = $this->reports_model->getBeginQtyOUTALL(0,$rw->id,$from_date2,$to_date2,$biller2);
									//$this->erp->print_arrays($beginOUTqty);
									foreach($propur as $rp){
										$btotal_qty = (isset($beginINqty[$rw->id][$rp->id])?$beginINqty[$rw->id][$rp->id]:0)-(isset($beginOUTqty[$rw->id][$rp->id])?$beginOUTqty[$rw->id][$rp->id]:0);
										//$begin_qty = $this->reports_model->getBeginQtyALL($rp->id,$rw->id,$from_date2,$to_date2,$biller2);
								
								?>
										<tr>
											<td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= $rp->name? "(" . $rp->code . ")" . $rp->name:$rp->id;?><?=" (".$rp->name_unit.")"?></td>
											<td style='text-align:right;'>
											<?php if($btotal_qty){?>
												<span style="color:blue;"><?=$btotal_qty?$this->erp->formatDecimal($btotal_qty):''?></span>
												<?php
												//if($begin_qty->bqty){
												//	echo  $this->erp->convert_unit_2_string($rp->id,$begin_qty->bqty);
												//}
										...
										$total_out = 0;
											...
											foreach($num as $tr){
												if($tr->transaction_type){
													$allqty = $this->reports_model->getQtyINALL($rp->id,$rw->id,$tr->transaction_type,$from_date2,$to_date2,$biller2);
													$qty_unit = $this->reports_model->getQtyUnitINALL($rp->id,$rw->id,$tr->transaction_type,$from_date2,$to_date2,$biller2);
											...
											<td style='text-align:right;'>
												...
												<span style="color:blue;"><?=$this->erp->formatDecimal($allqty->bqty)?></span>
													...
													<?php
													if($qty_unit->bqty){
														echo   $this->erp->convert_unit_2_string($rp->id,$qty_unit->bqty);
													}
												}
												?>
											</td>
											...
											if(is_array($num2)){
												...
												foreach($num2 as $tr2){
														if($tr2->transaction_type){
														$allqty2 = $this->reports_model->getQtyOUTALL($rp->id,$rw->id,$tr2->transaction_type,$from_date2,$to_date2,$biller2);
															$qty_unit2 = $this->reports_model->getQtyUnitOUTALL($rp->id,$rw->id,$tr2->transaction_type,$from_date2,$to_date2,$biller2);
														 ...
														 <?php
															 if($qty_unit2->bqty){
																echo   $this->erp->convert_unit_2_string($rp->id,$qty_unit2->bqty);
															 }
														 }
														?>
														 </td>
											...
											<?php
													 ...
													 if($am){
															...
															echo   $this->erp->convert_unit_2_string($rp->id,$am);
									...
									<?php
										...
										if(is_array($num)){
											foreach($num as $tr){
												if($tr->transaction_type){
													...
													$amount_qty = 888;// $this->reports_model->getAmountQtyINALL($product2,$rw->id,$tr->transaction_type,$rc->id,$from_date2,$to_date2,$biller2);
										...
										if(is_array($num2)){
											...
											foreach($num2 as $tr2){
													...
													if($tr2->transaction_type){
													 $amount_qty2 = 999;// $this->reports_model->getAmountQtyOUTALL($product2,$rw->id,$tr2->transaction_type,$rc->id,$from_date2,$to_date2,$biller2);
