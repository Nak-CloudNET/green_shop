	...
	*/
     ...
     * Edit Row Quantity Method 
     ...
     $(document).on("focus", '.rquantity, .received', function () {
        ...
        var row = $(this).closest('tr');
        var new_qty = parseFloat(row.find('.rquantity').val());
		item_id 						= row.attr('data-item-id');
	...
	$(document).on("change", '.received', function () {
		...
		if(row.find('.received').val() != NaN || row.find('.received').val() >= 0){
			var new_rqty = row.find('.received').val();
		}else{
			var new_rqty = new_qty;
		}
		
		poitems[item_id].row.received 	= new_rqty;
        poitems[item_id].row.received_status = 1;
        localStorage.setItem('poitems', JSON.stringify(poitems));
        loadItems();
	});
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            var product_id 		= item.row.id,
				...
				var received_status = item.row.received_status;
			...
			if(owner || admin || purchase_cost) {
				...
				tr_html += '<td class="text-right"><input class="rucost" name="unit_cost[]" type="hidden" value="' + net_unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + net_unit_cost + '"><input class="realcost" type="hidden" value="' + item.row.real_cost + '">';
			}
...
if(received_status == 0){
				tr_html += '<td class="text-right"><input name="received_status[]" type="hidden" value="' + 0 + '"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney( (parseFloat(item_cost) + parseFloat(pr_tax_val)) * item_qty ) + '</span></td>';
			}else{
				tr_html += '<td class="text-right"><input name="received_status[]" type="hidden" value="' + 1 + '"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney( (parseFloat(item_cost) + parseFloat(pr_tax_val)) * (qty_received == item_qty? item_qty:qty_received) ) + '</span></td>';
			}
			...
			if(received_status == 0){
				if(item_qty > 0){
					total += formatPurDecimal( (parseFloat(item_cost) + parseFloat(pr_tax_val)) * item_qty );
				}else{
					total +=0;
				}
			}else{
